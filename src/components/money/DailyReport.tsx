...
const DailyReport = ({ shops, defaultShop, allowAll = false }: { shops: Shop[]; defaultShop?: string; allowAll?: boolean }) => {
  ...
  const customersToday = useMemo(() => {
    const byCustomer = new Map<string, { customer: string; lines: { product: string; unit: string; qty: number }[] }>();
    tx.forEach(t => {
      const name = t.customer_name || '—';
      const key = name.toLowerCase();
      if (!byCustomer.has(key)) byCustomer.set(key, { customer: name, lines: [] });
      const bucket = byCustomer.get(key)!;
      items.filter(it => it.transaction_id === t.id).forEach(it => {
        const existing = bucket.lines.find(l => l.product === it.product && l.unit === it.unit);
        if (existing) existing.qty += Number(it.quantity || 0);
        else bucket.lines.push({ product: it.product, unit: it.unit, qty: Number(it.quantity || 0) });
      });
    });
    return [...byCustomer.values()].sort((a, b) => a.customer.localeCompare(b.customer));
  }, [tx, items]);

  // Money by product × unit
  const moneyPivot = useMemo(() => {
    const products = new Map<string, Map<string, number>>();
    const unitSet = new Set<string>();
    items.forEach(it => {
      unitSet.add(it.unit);
    ...
    const rows = [...products.entries()].map(([product, m]) => {
      const byUnit: Record<string, number> = {};
      let total = 0;
      units.forEach(u => { byUnit[u] = m.get(u) || 0; total += byUnit[u]; });
      return { product, byUnit, total };
    }).sort((a, b) => b.total - a.total);
    const colTotals: Record<string, number> = {};
    units.forEach(u => { colTotals[u] = rows.reduce((s, r) => s + (r.byUnit[u] || 0), 0); });
    const grand = rows.reduce((s, r) => s + r.total, 0);
    return { units, rows, colTotals, grand };
  }, [items]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader><CardTitle>Customers who bought today</CardTitle></CardHeader>
        <CardContent className="overflow-x-auto">
          {customersToday.length === 0 ? (
            <div className="text-sm text-muted-foreground">No customer purchases on this date.</div>
          ) : (
            <Table>
              <TableHeader><TableRow><TableHead>Customer</TableHead><TableHead>Purchases</TableHead></TableRow></TableHeader>
              <TableBody>
                {customersToday.map(c => (
                  <TableRow key={c.customer}>
                    <TableCell className="font-semibold align-top whitespace-nowrap">{c.customer}</TableCell>
                    <TableCell className="text-sm">
                      {c.lines.map((l, i) => (
                        <span key={i}>
                          <span className="font-semibold">{l.qty}</span> {l.product} <span className="text-muted-foreground">({l.unit})</span>{i < c.lines.length - 1 ? ', ' : ''}
                        </span>
                      ))}
                    </TableCell>
                  </TableRow>
      ...
      <Card>
        <CardHeader><CardTitle>Money generated by product &times; unit</CardTitle></CardHeader>
        <CardContent className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Product</TableHead>
              {moneyPivot.units.map(u => <TableHead key={u} className="text-right">{u}</TableHead>)}
              <TableHead className="text-right">Total</TableHead>
            </TableRow></TableHeader>
            <TableBody>
              {moneyPivot.rows.map(r => (
                <TableRow key={r.product}>
                  <TableCell className="font-medium">{r.product}</TableCell>
                  {moneyPivot.units.map(u => <TableCell key={u} className="text-right tabular-nums">{r.byUnit[u] ? Math.round(r.byUnit[u]).toLocaleString() : ''}</TableCell>)}
                  <TableCell className="text-right font-semibold tabular-nums">{Math.round(r.total).toLocaleString()}</TableCell>
                </TableRow>
              ))}
              {moneyPivot.rows.length === 0 && (
                <TableRow><TableCell colSpan={moneyPivot.units.length + 2} className="text-center text-muted-foreground">No sales for this day.</TableCell></TableRow>
              )}
              {moneyPivot.rows.length > 0 && (
                <TableRow className="font-semibold bg-muted/40">
                  <TableCell>Total</TableCell>
                  {moneyPivot.units.map(u => <TableCell key={u} className="text-right tabular-nums">{Math.round(moneyPivot.colTotals[u]).toLocaleString()}</TableCell>)}
                  <TableCell className="text-right tabular-nums">{Math.round(moneyPivot.grand).toLocaleString()}</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
